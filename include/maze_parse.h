...
namespace mazegen
{
  ...
  /// (bottom) edge and col == 0 is the western (left) edge — matching the
  ...
  struct Maze
  {
    ...
    ///
    /// hWall[col][r] is true when a wall runs along latitude r, separating
    /// row r-1 from row r. r ranges over [0, rows]: r==0 is the south outer
    /// edge and r==rows the north outer edge.
    std::vector<std::vector<bool>> hWall;

    /// \brief Vertical (north–south) wall edges, indexed vWall[c][row].
    ///
    /// vWall[c][row] is true when a wall runs along longitude c, separating
    /// column c-1 from column c. c ranges over [0, cols]: c==0 is the west
    /// outer edge and c==cols the east outer edge.
    ...
    ///
    /// A classic 16×16 micromouse goal is a 2×2 block, so this commonly
    /// holds up to four entries.
    std::vector<std::pair<int, int>> goalCells;
  };

  /// \brief Parse a micromouseonline-format maze file into a Maze struct.
  ///
  /// The file format alternates post rows (even text lines) and wall rows
  /// (odd text lines). Posts are marked 'o', horizontal walls by '-', vertical
  /// walls by '|', and cell markers by 'S' (start) or 'G' (goal).
  ///
  /// \param[in] _path Filesystem path to the maze text file.
  /// \return Populated Maze with walls, start, and goal cells set.
